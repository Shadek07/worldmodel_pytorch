...
from vae.vae_torch import VAE, save_model, load_model
from rnn.rnn_torch import hps_sample, MDNRNN, rnn_init_state, rnn_next_state, rnn_output, get_rnn_output_size, load_rnn_model
import torch
...
def make_model(load_model=True):
  ...
  model = Model(is_load_model=load_model)
  return model
...
class Model:
  ...
  def __init__(self, is_load_model=True):
    self.env_name = "carracing"

    self.device = torch.device('cpu') #'cuda' if torch.cuda.is_available() else
    self.vae = VAE(z_size=z_size, kl_tolerance=kl_tolerance, learning_rate=vae_learning_rate).to(self.device)
    self.rnn_model = MDNRNN(rnn_input_size, self.device, rnn_output_size=get_rnn_output_size).to(self.device)
    self.optimizer = optim.Adam(self.rnn_model.parameters(), lr=rnn_learning_rate)

    if is_load_model:
      self.vae.load_state_dict(torch.load("tf_vae/vae.pt", map_location=self.device)) #load_model('vae/vae.pt', self.vae, self.device)
      self.vae.to(self.device)
      self.vae.eval()
      self.rnn_model.load_state_dict(torch.load("tf_rnn/rnn.pt", map_location=self.device))
      self.rnn_model.to(self.device)
      self.rnn_model.eval()

    self.state = self.rnn_model.init_hidden(1) #batch_size=1 #rnn_init_state(self.rnn_model)
    self.rnn_mode = True

    self.input_size = get_rnn_output_size(EXP_MODE)
    self.z_size = 32
  ...
  def reset(self):
    self.state = self.rnn_model.init_hidden(1)  #rnn_init_state(self.rnn_model)

  def encode_obs(self, obs):
    # convert raw obs to z, mu, logvar
    result = np.copy(obs).astype(np.float)/255.0
    result = result.reshape(1, 3, 64, 64)
    result = torch.tensor(result, dtype=torch.float)
    mu, logvar = self.vae.encoder(result)
    z = self.vae.reparameterize(mu, logvar)

    return z, mu.detach().numpy(), logvar.detach().numpy()

  def get_action(self, z):
    h = rnn_output(self.state, z, EXP_MODE)
    #print('h size', h.size()) #h size torch.Size([1, 288])
    h = h.detach().numpy()
    if EXP_MODE == MODE_Z_HIDDEN: # one hidden layer
    ...
    self.state = rnn_next_state(self.rnn_model, z, action, self.state)
    #print(self.state[0].size(), self.state[1].size()) #torch.Size([1, 1, 256]) torch.Size([1, 1, 256])
    return action
  ...
  def init_random_model_params(self, stdev=0.1):
    ...
    rnn_params = self.rnn_model.get_random_model_params(stdev=stdev)
    self.rnn_model.set_model_params(rnn_params)
