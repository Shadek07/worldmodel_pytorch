...
def save_model(path, mdnrnn, optimizer):
  torch.save({
...
def load_rnn_model(path, mdnrnn, optimizer, device):
  checkpoint = torch.load(path, map_location=device)
  mdnrnn.load_state_dict(checkpoint['model_state_dict'])
  optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
  #mdnrnn.train()
  return mdnrnn, optimizer


class MDNRNN(nn.Module):
  def __init__(self, z_size, device, n_hidden=256, num_mixture=5, n_layers=1, rnn_output_size=32):
    ...
    self.lstm = nn.LSTM(z_size, n_hidden, n_layers)
    self.dense = nn.Linear(self.n_hidden, 5*32*3) #self.num_mixture*(self.rnn_output_size*3)
  ...
  def get_mixture_coef(self, output, num_mixtures):
    logmix, mean, logstd = torch.split(output, num_mixtures, dim=1)
    logmix = logmix - self.logsumexp(logmix, dim=1, keepdim=True)
    return logmix, mean, logstd

  def lognormal(self, y, mean, logstd):
    #print(y.size(), mean.size(), logstd.size())
    return -0.5 * ((y - mean) / logstd.exp()) ** 2 - logstd - logstd.new([np.log(np.sqrt(2.0 * np.pi))]) #logSqrtTwoPI

  def get_lossfunc(self, logmix, mean, logstd, y):
    y = y.view(-1, 1)
    v = logmix + self.lognormal(y, mean, logstd)
    v = self.logsumexp(v, dim=1, keepdim=True) #tf.reduce_logsumexp(v, 1, keepdims=True)
    return -v.mean()

  def forward(self, x, h):
    # Forward propagate LSTM
    #x size = seq_len, batch_size, input_size = 1000, 100, 35
    # h is tuple of two tensors- each with size (1, 100, 256)
    output, (h, c) = self.lstm(x, h) #y size = (num_batch, time_step, n_hidden) = 100, 1000, 256, h size = (1, 100, 256)
    output = output.reshape(-1, self.n_hidden)
    output = self.dense(output)
    output = output.view(-1, self.num_mixture*3)
    pi, mu, sigma = self.get_mixture_coef(output, self.num_mixture)
    return (pi, mu, sigma), (h, c)

  def init_hidden(self, num_batch):
    return (torch.zeros(self.n_layers, num_batch, self.n_hidden).to(self.device, dtype=torch.float),
            torch.zeros(self.n_layers, num_batch, self.n_hidden).to(self.device, dtype=torch.float))
...
'''class MDNRNN():
  ...
  def build_model(self, hps):
    ...
    self.input_x = tf.placeholder(dtype=tf.float32, shape=[self.hps.batch_size, self.hps.max_seq_len, INWIDTH]) #100x1000x35
    self.output_x = tf.placeholder(dtype=tf.float32, shape=[self.hps.batch_size, self.hps.max_seq_len, OUTWIDTH]) #100x1000x32

    actual_input_x = self.input_x #100x1000x35
    self.initial_state = cell.zero_state(batch_size=hps.batch_size, dtype=tf.float32) 

    NOUT = OUTWIDTH * KMIX * 3 #32x5x3

    with tf.variable_scope('RNN'):
      output_w = tf.get_variable("output_w", [self.hps.rnn_size, NOUT]) #(256, 32x5x3)
    ...
    output = tf.reshape(output, [-1, KMIX * 3]) #(-1, 5x3)
...
def rnn_next_state(rnn_model, z, a, prev_state):
  #hidden = rnn_model.init_hidden(1)  # ( (1x100x256), (1x100x256))
  a = torch.tensor(a, dtype=torch.float)
  z = z.view(1, 1, 32)
  a = a.view(-1, 1, 3)
  inputs = torch.cat((z, a), dim=2)
  inputs = inputs.view(-1, 1, 35)#inputs.view(-1, hps.batch_size, rnn_input_size)
  (pi, mu, sigma), next_state = rnn_model.forward(inputs, prev_state) #(pi, mu, sigma), (h, c) = rnn_model(inputs, prev_state)
  return next_state

def get_rnn_output_size(mode):
  if mode == MODE_ZCH:
    return (32+256+256)
...
def rnn_output(state, z, mode):
  if mode == MODE_ZCH:
    return np.concatenate([z, np.concatenate((state[1], state[0]), axis=1)[0]])
  if mode == MODE_ZC:
    return np.concatenate([z, state[1]]) #c
  if mode == MODE_ZH:
    #return np.concatenate([z, state[0]]) #h
    return torch.cat((z, state[0].view(1, -1)), dim=1)
